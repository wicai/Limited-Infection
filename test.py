...
def full_infect_test():
    """Tests full_infection module on a variety of graphs.
    
    """
    print "full_infection + graph of 100 nodes where each edge has ~10% of appearing"
    num_users = 100
    rand = cg.random_graph(num_users, .05)
    full_infection(rand[0], 1)
    check_graph(rand)
    print
    print "full_infection + empty graph with 100 nodes"
    empty_graph = cg.empty_graph(100)
    full_infection(empty_graph[0], 1)
    check_graph(empty_graph)
    print
    print "full_infection + class graph with 20 teachers, avg of 15 students" \
        + "std dev of 5"
    teach = cg.class_graph(20, 15, 5)
    full_infection(teach[0],1 )
    check_graph(teach)
    print
    print "full_infection + full graph with 1000 nodes"
    full = cg.full_graph(1000)
    full_infection(full[random.randint(0,999)], 1)
    check_graph(full)
